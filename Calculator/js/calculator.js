...
const Calculator = {
    ...
    First_Operand: null,
    //checks whether or not the second operand has been inputed by the user
    Wait_Second_Operand: false,
    //holds the operator, we set it to null
    operator: null,
};
...
function Input_Digit(digit) {
    ...
    if (Wait_Second_Operand === true) {
        ...
        //overwrites display value if the current value is 0
...
function Input_Decimal(dot) {
    ...
    if(!Calculator.Display_Value.includes(dot)) {
        //if the display value does not contain a decimal point
...
function Update_Display() {
    ...
    //input tag iin the html document
...
keys.addEventListener('click', (event) => {
    //the target vairable is an object that represents the element
    //that was clicked
    const {target} = event;
